...
def increment(session, subject, amount):
    # retreive subject, add amount to karma value, create new subject with [amount] karma if necesary
...
def decrement(session, subject, amount):
    # retreive subject, subtract amount from karma value, create new subject with -[amount] karma if necesary
...
def get_top(session, num_to_select):
    top_scores = session.query(Karma.karma).order_by(Karma.karma.desc()).distinct().limit(num_to_select)
    entries = session.query(Karma).filter(Karma.karma.in_(top_scores)).order_by(Karma.karma.desc())
